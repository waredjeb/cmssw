...
namespace ticl {
  ...
  inline unsigned getLayerFromLC(const reco::CaloClusterSoAConstView &layerClusters,
                                 unsigned i,
                                 const hgcal::RecHitTools &rhtools) {
    return rhtools.getLayerWithOffset(layerClusters.indexes()[i].seedID());
  }
  ...
  inline std::vector<std::vector<unsigned>> sortByLayer(const Trackster &ts,
    ...
    for (unsigned i = 0; i < N; ++i) {
      auto layer = getLayerFromLC(layerClusters, ts.vertices(i), rhtools);
