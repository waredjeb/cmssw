...
LinkingAlgoByGraph::LinkingAlgoByGraph(const edm::ParameterSet &conf)
...
LinkingAlgoByGraph::~LinkingAlgoByGraph() {}

void LinkingAlgoByGraph::initialize(const HGCalDDDConstants *hgcons,
...
float LinkingAlgoByGraph::findSkeletonPoints(float percentage,
  ...
  if (LinkingAlgoBase::algo_verbosity_ > VerbosityLevel::Advanced) {
    for (size_t iLay = 0; iLay < energyInLayer.size(); ++iLay) {
      LogDebug("LinkingAlgoByGraph") << "Layer " << iLay << " contains a Trackster energy fraction of "
  ...
  if (LinkingAlgoBase::algo_verbosity_ > VerbosityLevel::Advanced) {
    for (size_t iLay = 0; iLay < cumulativeEnergyInLayer.size(); ++iLay) {
      LogDebug("LinkingAlgoByGraph") << "Layer " << iLay << " cumulative has a Trackster energy fraction of "
  ...
  if (layerI != cumulativeEnergyInLayer.end()) {
    ...
    if (LinkingAlgoBase::algo_verbosity_ > VerbosityLevel::Advanced) {
      LogDebug("LinkingAlgoByGraph") << "Layer containing at least an energy fraction " << percentage << " is " << layer
                                     << "\n";
    }
...
void LinkingAlgoByGraph::buildLayers() {
...
void LinkingAlgoByGraph::linkTracksters(const edm::Handle<std::vector<reco::Track>> tkH,
  ...
  const auto &layerClusters = *layerClustersH;
  ...
  if (LinkingAlgoBase::algo_verbosity_ > VerbosityLevel::Advanced)
    LogDebug("LinkingAlgoByGraph") << "------- Graph Linking ------- \n";
