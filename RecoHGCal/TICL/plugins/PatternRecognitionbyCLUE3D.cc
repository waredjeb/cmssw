...
void PatternRecognitionbyCLUE3D<TILES>::makeTracksters(
  ...
  std::vector<std::vector<int>> tmp_tracksterSeedsDoublets;
  ...
  for (auto const &lc : input.layerClusters) {
    ...
    // The case of single cell layer clusters has to handled differently.
    ...
    //reweighting the energy using the mask.
  ...
  tmp_tracksterSeedsDoublets.reserve(nTracksters);
    ...
    for (unsigned int lc = 0; lc < thisLayer.x.size(); ++lc) {
      ...
      if (thisLayer.clusterIndex[lc] >= 0) {
        ...
        if (thisLayer.isSeed[lc]) {
          ...
          tmp_tracksterSeedsDoublets.emplace_back(tmp_followers);
        }
        tmp_result[thisLayer.clusterIndex[lc]].vertices().push_back(thisLayer.layerClusterOriginalIdx[lc]);
        tmp_result[thisLayer.clusterIndex[lc]].vertex_multiplicity().push_back(input.mask[thisLayer.layerClusterOriginalIdx[lc]]);
        // loop over followers
        for (auto [follower_lyrIdx, follower_soaIdx] : thisLayer.followers[lc]) {
  ...
  for (size_t i = 0; i < tmp_result.size(); i++) {
    if (static_cast<int>(tmp_result[i].vertices().size()) >= minNumLayerCluster_) {
      ...
      tracksterSeedsDoublets.push_back(tmp_tracksterSeedsDoublets[i]);
    }
  }
