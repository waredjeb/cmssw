...
namespace {
  ...
  float projective_distance(const ticl::Vector &point1, const ticl::Vector &point2) {
    ...
    LogDebug("TracksterLinkinbySkeletons") << "Computing distance between point : " << point1 << " And point "
...
bool TracksterLinkingbySkeletons::areCompatible(const ticl::Trackster &myTrackster,
  ...
  if (!isGoodTrackster(myTrackster, mySkeleton, min_num_lcs_, min_trackster_energy_, pca_quality_th_)) {
    LogDebug("TracksterLinkinbySkeletons") << "Inner Trackster with energy " << myTrackster.raw_energy() << " Num LCs "
                                            << myTrackster.vertices().size() << " NOT GOOD " << std::endl;
    return false;
  } else {
    LogDebug("TracksterLinkinbySkeletons") << "Inner Trackster wi energy " << myTrackster.raw_energy() << " Num LCs "
    ...
    LogDebug("TracksterLinkinbySkeletons")
    ...
    if (isGoodTrackster(otherTrackster, otherSkeleton, min_num_lcs_, min_trackster_energy_, pca_quality_th_)) {
      ...
      if (areAlignedInProjectiveSpace) {
        ...
        if (alignedSkeletons)
          LogDebug("TracksterLinkinbySkeletons") << "\t\t Linked! " << std::endl;
        ...
        if (minDistance_z < min_distance_z_[isEE]) {
          ...
          LogDebug("TracksterLinkinbySkeletons") << "\t\t Not Linked Distance Z " << minDistance_z << std::endl;
          return false;
        }
      }
    } else {
      ...
      if (areAlignedInProjectiveSpace) {
        ...
        if (d < max_distance_projective_sqr_closest_points_[isEE] and
          ...
          LogDebug("TracksterLinkinbySkeletons") << "\t\t\t Linked! " << d << std::endl;
          return true;
        } else {
          LogDebug("TracksterLinkinbySkeletons") << "Distance between closest point " << d << " Distance in z "
                                                  << max_z_distance_closest_points_[isEE] << std::endl;
          bool isInCyl = isInCylinder(mySkeleton, otherSkeleton, cylinder_radius_sqr_[isEE]);
          LogDebug("TracksterLinkinbySkeletons") << "Two Points are in Cylinder  " << isInCyl << std::endl;
          if (isInCyl) {
            LogDebug("TracksterLinkinbySkeletons") << "\t\t\t Linked! " << d << std::endl;
          }
          return isInCyl;
...
void TracksterLinkingbySkeletons::linkTracksters(
  ...
  LogDebug("TracksterLinkinbySkeletons") << "****************  FINAL GRAPH **********************" << std::endl;
  ...
  LogDebug("TracksterLinkinbySkeletons") << "********************************************************" << std::endl;
  ...
  for (auto const &comp : components) {
    LogDebug("TracksterLinkinbySkeletons") << "Component " << ic << " Node: ";
    ...
    for (auto const &node : comp) {
      LogDebug("TracksterLinkinbySkeletons") << node << " ";
