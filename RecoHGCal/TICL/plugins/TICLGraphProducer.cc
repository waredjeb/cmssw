...
public:
  ...
  void beginJob();
  void endJob();
  bool isPointInCone(const Vector &coneOrigin,
...
bool TICLGraphProducer::isPointInCone(const Vector &coneOrigin,
                                      const Vector &direction,
                                      const float halfAngle,
                                      float maxHeight,
                                      const Vector &testPoint) {
  //std::cout << "Test Point " << testPoint << " coneOrigin " << coneOrigin << std::endl;
  Vector toCheck = testPoint - coneOrigin;
...
void TICLGraphProducer::produce(edm::Event &evt, const edm::EventSetup &es) {
  ...
  auto findPoint =
   ...
   auto returnSkeletons = [&](const Trackster& trackster) -> std::array<Vector, 3>{
    ...
    std::sort(skeleton.begin(), skeleton.end(), [](Vector& v1, Vector& v2){
               return v1.Z() < v2.Z();
            });
    return skeleton;
  ...
  const float halfAngle1 = 0.523599; //30 degrees
  const float halfAngle2 = 0.349066; //22 degrees
  for (size_t id_t = 0; id_t < trackstersclue3d.size(); ++id_t) {
    auto t = trackstersclue3d[id_t];
      ...
      for (int eta_i = search_box[0]; eta_i <= search_box[1]; ++eta_i) {
        for (int phi_i = search_box[2]; phi_i <= search_box[3]; ++phi_i) {
          ...
          for (auto n : neighbours) {
            ...
            if(isPointInCone(skeleton[0], tracksterEigenvec*tracksterEigenVal,  halfAngle1, maxHeight, skeleton2[0])){ //first cone
                allNodes2[id_t].addNeighbour(n); 
                allNodes2[n].addNeighbour(id_t); 
            } 
            if(isPointInCone(skeleton[2], t.eigenvectors(0), halfAngle2, 500.f, skeleton2[0])){ //second cone
                allNodes2[id_t].addNeighbour(n); 
                allNodes2[n].addNeighbour(id_t); 
            } 
          }
  ...
  auto resultGraphCone = std::make_unique<TICLGraph>(allNodes2);
