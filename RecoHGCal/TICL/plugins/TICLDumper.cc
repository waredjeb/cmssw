...
private:
  ...
  std::vector<std::vector<uint32_t>> tracksters_recoToSim_PU;
  std::vector<std::vector<float>> tracksters_recoToSim_PU_score;
  std::vector<std::vector<float>> tracksters_recoToSim_PU_sharedE;
  std::vector<std::vector<uint32_t>> tracksters_simToReco_PU;
  std::vector<std::vector<float>> tracksters_simToReco_PU_score;
  std::vector<std::vector<float>> tracksters_simToReco_PU_sharedE;
...
void TICLDumper::clearVariables() {
  ...
  tracksters_recoToSim_PU.clear();
  tracksters_recoToSim_PU_score.clear();
  tracksters_recoToSim_PU_sharedE.clear();
  tracksters_simToReco_PU.clear();
  tracksters_simToReco_PU_score.clear();
  tracksters_simToReco_PU_sharedE.clear();
...
void TICLDumper::beginJob() {
  ...
  if (saveAssociations_) {
    ...
    associations_tree_->Branch("tracksters_recoToSim_PU", &tracksters_recoToSim_PU);
    associations_tree_->Branch("tracksters_recoToSim_PU_score", &tracksters_recoToSim_PU_score);
    associations_tree_->Branch("tracksters_recoToSim_PU_sharedE", &tracksters_recoToSim_PU_sharedE);
    associations_tree_->Branch("tracksters_simToReco_PU", &tracksters_simToReco_PU);
    associations_tree_->Branch("tracksters_simToReco_PU_score", &tracksters_simToReco_PU_score);
    associations_tree_->Branch("tracksters_simToReco_PU_sharedE", &tracksters_simToReco_PU_sharedE);
  }
...
void TICLDumper::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  ...
  // Tackster reco->sim associations
  tracksters_recoToSim_PU.resize(tracksters.size());
  tracksters_recoToSim_PU_score.resize(tracksters.size());
  tracksters_recoToSim_PU_sharedE.resize(tracksters.size());
  for (size_t i = 0; i < tracksters.size(); ++i) {
    const edm::Ref<ticl::TracksterCollection> tsRef(tracksters_handle, i);
    ...
    if (stsPU_iter != tsRecoSimPUMap.end()) {
      ...
      for (auto& sts : stsPUassociated) {
        auto sts_id = (sts.first).get() - (edm::Ref<ticl::TracksterCollection>(simTrackstersPU_h, 0)).get();
        tracksters_recoToSim_PU[i].push_back(sts_id);
        tracksters_recoToSim_PU_score[i].push_back(sts.second.second);
        tracksters_recoToSim_PU_sharedE[i].push_back(sts.second.first);
      }
    }
  }
  ...
  tracksters_simToReco_PU.resize(nsimTrackstersPU);
  tracksters_simToReco_PU_score.resize(nsimTrackstersPU);
  tracksters_simToReco_PU_sharedE.resize(nsimTrackstersPU);
  for (size_t i = 0; i < nsimTrackstersPU; ++i) {
    const edm::Ref<ticl::TracksterCollection> stsPURef(simTrackstersPU_h, i);
    ...
    if (ts_iter != tsSimToRecoPUMap.end()) {
      ...
      for (auto& ts : tsAssociated) {
        auto ts_idx = (ts.first).get() - (edm::Ref<ticl::TracksterCollection>(tracksters_handle, 0)).get();
        tracksters_simToReco_PU[i].push_back(ts_idx);
        tracksters_simToReco_PU_score[i].push_back(ts.second.second);
        tracksters_simToReco_PU_sharedE[i].push_back(ts.second.first);
      }
    }
  }
...
void TICLDumper::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("simToRecoAssociatorPU",
                          edm::InputTag("tracksterSimTracksterAssociationLinkingbyCLUE3DPU", "simToReco"));
