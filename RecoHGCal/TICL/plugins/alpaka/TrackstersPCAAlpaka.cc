...
void ticlAlpaka::assignPCAtoTracksters(std::vector<ticl::Trackster> &tracksters,
  ...
  for (auto &trackster : tracksters) {
    ...
    if (computeLocalTime)
      timeTrackster = ticlAlpaka::computeLocalTracksterTime(trackster, layerClusters, layerClustersTime, barycenter, N);
    else
      timeTrackster = ticlAlpaka::computeTracksterTime(trackster, layerClustersTime, N);
...
std::pair<float, float> ticlAlpaka::computeLocalTracksterTime(const ticl::Trackster &trackster,
                                                        const std::vector<reco::CaloCluster> &layerClusters,
                                                        const edm::ValueMap<std::pair<float, float>> &layerClustersTime,
...
std::pair<float, float> ticlAlpaka::computeTracksterTime(const ticl::Trackster &trackster,
                                                   const edm::ValueMap<std::pair<float, float>> &layerClustersTime,
                                                   size_t N) {
