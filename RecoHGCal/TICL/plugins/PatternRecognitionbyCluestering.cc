...
template <typename TILES>
void PatternRecognitionbyCluestering<TILES>::dumpTiles(const TILES &tiles) const {
...
template <typename TILES>
void PatternRecognitionbyCluestering<TILES>::dumpTracksters(const std::vector<std::pair<int, int>> &layerIdx2layerandSoa,
  ...
  std::cout << "****************IN CLUESTERING**********************" << std::endl;
  int num = 0;
  ...
  for (auto const &t : tracksters) {
    for (auto v : t.vertices()) {
      ...
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCluestering_NTP")
...
template <typename TILES>
void PatternRecognitionbyCluestering<TILES>::dumpClusters(const TILES &tiles,
                                                          const std::vector<std::pair<int, int>> &layerIdx2layerandSoa,
                                                          const int eventNumber) const {
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    edm::LogVerbatim("PatternRecognitionbyCluestering")
        << "[evt, lyr, Seed,      x,       y,       z, r/|z|,   eta,   phi, "
...
void PatternRecognitionbyCluestering<TILES>::makeTracksters(
  ...
  std::cout << "****************MAKING TRACKSTERS IN CLUESTERING**********************" << std::endl;
  ...
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    edm::LogVerbatim("PatternRecognitionbyCluestering") << "New Event";
  }
  ...
  for (unsigned int i = 0; i < rhtools_.lastLayer(); ++i) {
    ...
    if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
      edm::LogVerbatim("PatternRecognitionbyCluestering") << "Layer " << i << " located at Z: " << layersPosZ_.back();
    }
  }
  ...
  for (auto const &lc : input.layerClusters) {
    if (input.mask[layerIdx] == 0.) {
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCluestering") << "Skipping masked cluster: " << layerIdx;
      }
    ...
    if (invClsize == 1.) {
      ...
      if (rhtools_.isSilicon(detId)) {
        ...
        if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
          edm::LogVerbatim("PatternRecognitionbyCluestering") << "Single cell cluster in silicon, rx: " << std::setw(5)
  ...
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    edm::LogVerbatim("PatternRecognitionbyCluestering")
        << "Reconstructed " << nTracksters << " tracksters" << std::endl;
  ...
  for (unsigned int layer = 0; layer < clusters_.size(); ++layer) {
    ...
    if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
      edm::LogVerbatim("PatternRecognitionbyCluestering") << "Examining Layer: " << layer;
    }
    for (unsigned int lc = 0; lc < thisLayer.x.size(); ++lc) {
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCluestering") << "Trackster " << thisLayer.clusterIndex[lc];
      }
  ...
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    for (auto const &t : result) {
      edm::LogVerbatim("PatternRecognitionbyCluestering") << "Barycenter: " << t.barycenter();
      edm::LogVerbatim("PatternRecognitionbyCluestering") << "LCs: " << t.vertices().size();
      edm::LogVerbatim("PatternRecognitionbyCluestering") << "Energy: " << t.raw_energy();
      edm::LogVerbatim("PatternRecognitionbyCluestering") << "Regressed: " << t.regressed_energy();
    }
  }
  ...
  reset();
  if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
    edm::LogVerbatim("PatternRecognitionbyCluestering") << std::endl;
  }
}

template <typename TILES>
void PatternRecognitionbyCluestering<TILES>::energyRegressionAndID(const std::vector<reco::CaloCluster> &layerClusters,
...
template <typename TILES>
void PatternRecognitionbyCluestering<TILES>::calculateLocalDensity(
  ...
  for (unsigned int i = 0; i < numberOfClusters; i++) {
    ...
    for (int currentLayer = minLayer; currentLayer <= maxLayer; currentLayer++) {
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCluestering") << "RefLayer: " << layerId << " SoaIDX: " << i;
        edm::LogVerbatim("PatternRecognitionbyCluestering") << "NextLayer: " << currentLayer;
      }
      const auto &tileOnLayer = tiles[currentLayer];
      bool onSameLayer = (currentLayer == layerId);
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCluestering") << "onSameLayer: " << onSameLayer;
      }
      ...
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
        edm::LogVerbatim("PatternRecognitionbyCluestering") << "eta: " << clustersOnLayer.eta[i];
        edm::LogVerbatim("PatternRecognitionbyCluestering") << "phi: " << clustersOnLayer.phi[i];
        edm::LogVerbatim("PatternRecognitionbyCluestering")
            << "etaBinMin: " << etaBinMin << ", etaBinMax: " << etaBinMax;
        edm::LogVerbatim("PatternRecognitionbyCluestering")
            << "phiBinMin: " << phiBinMin << ", phiBinMax: " << phiBinMax;
      }
      for (int ieta = etaBinMin; ieta <= etaBinMax; ++ieta) {
        auto offset = ieta * nPhiBin;
        if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
          edm::LogVerbatim("PatternRecognitionbyCluestering") << "offset: " << offset;
        }
        for (int iphi_it = phiBinMin; iphi_it <= phiBinMax; ++iphi_it) {
          int iphi = ((iphi_it % nPhiBin + nPhiBin) % nPhiBin);
          if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
            edm::LogVerbatim("PatternRecognitionbyCluestering") << "iphi: " << iphi;
            edm::LogVerbatim("PatternRecognitionbyCluestering")
          ...
          for (auto otherClusterIdx : tileOnLayer[offset + iphi]) {
            ...
            if ((layerandSoa.first == -1) && (layerandSoa.second == -1)) {
              if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
                edm::LogVerbatim("PatternRecognitionbyCluestering") << "Skipping masked layerIdx " << otherClusterIdx;
              }
              continue;
            }
            ...
            if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > VerbosityLevel::Advanced) {
              ...
              edm::LogVerbatim("PatternRecognitionbyCluestering") << "Distance[cm]: " << (dist * clustersOnLayer.z[i]);
              edm::LogVerbatim("PatternRecognitionbyCluestering")
                  << "Energy Other:   " << clustersLayer.energy[layerandSoa.second];
              edm::LogVerbatim("PatternRecognitionbyCluestering") << "Cluster radius: " << clustersOnLayer.radius[i];
            }
            if (reachable) {
...
template <typename TILES>
void PatternRecognitionbyCluestering<TILES>::calculateDistanceToHigher(
    const TILES &tiles, const int layerId, const std::vector<std::pair<int, int>> &layerIdx2layerandSoa) {
  constexpr int nEtaBin = TILES::constants_type_t::nEtaBins;
  constexpr int nPhiBin = TILES::constants_type_t::nPhiBins;
  auto &clustersOnLayer = clusters_[layerId];
...
template <typename TILES>
int PatternRecognitionbyCluestering<TILES>::findAndAssignTracksters(
...
template class ticl::PatternRecognitionbyCluestering<TICLLayerTilesHFNose>;
