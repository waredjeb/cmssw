...
void HGCFEElectronics<DFr>::runShaperWithToT(DFr& dataFrame,
  ...
  for (int it = 0; it < (int)(chargeColl.size()); ++it) {
    ...
    if (ft>it && ft<static_cast<int>(newCharge_.size()) && tdcChargeDrainParameterisation_.size() > tdcLeakageTauIdx_) {
      const float deltaT2nextBx((busyBxs * 25.0f - integTime));
      const float tdcOnsetLeakage(tdcOnset * vdt::fast_expf(-deltaT2nextBx / tdcChargeDrainParameterisation_[tdcLeakageTauIdx_]));
      if (debug)
        edm::LogVerbatim("HGCFE")
          << "\t Leaking remainder of TDC onset " << tdcOnset << " fC, to be dissipated in "
                  << deltaT2nextBx << " DeltaT/tau=" << deltaT2nextBx << " / "
                  << tdcChargeDrainParameterisation_[tdcLeakageTauIdx_] << " ns, adds " << tdcOnsetLeakage << " fC @ "
