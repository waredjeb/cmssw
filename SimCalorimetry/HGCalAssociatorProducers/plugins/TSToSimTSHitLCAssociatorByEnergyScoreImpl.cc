...
hgcal::association_t TSToSimTSHitLCAssociatorByEnergyScoreImpl::makeConnections(
  ...
  for(size_t i =0; i< nTracksters; ++i) recoToSim_sharedEnergyAndScore[i].resize(nSimTracksters);
  for(size_t i =0; i< nSimTracksters; ++i) simToReco_sharedEnergyAndScore[i].resize(nTracksters);
  ...
  { 
    ...
    { 
      for(const auto& haf : sc->hits_and_fractions())
      {
        ...
        auto found = std::find_if(detIdCaloParticleId_Map[hitId].begin(), detIdCaloParticleId_Map[hitId].end(), [=](const std::pair<int, float>& v) { return v.first == static_cast<int>(i); });
    ...
    for(const auto& v: lcsInSimTrackster)
    {
      for(const auto& haf: layerClusters[v].hitsAndFractions())
      {
        ...
        if(simTSs[i].seedID() == cPHandle_id){
          found = std::find_if(detIdSimTSId_Map[hitId].begin(), detIdSimTSId_Map[hitId].end(), [=](const std::pair<int, float>& v) { return v.first == seedIndex; });
          ...
          found = std::find_if(detIdSimClusterId_Map[hitId].begin(), detIdSimClusterId_Map[hitId].end(), [=](const std::pair<int, float>& v) { return v.first == seedIndex; });
        ...
        for(size_t j = 0; j< nTracksters; ++j)
        {
          ...
          auto found_reco = std::find_if(detIdToRecoTSId_Map[hitId].begin(), detIdToRecoTSId_Map[hitId].end(), [=](const std::pair<int, float>& v) { return v.first == static_cast<int>(j); });
          if (found_reco != detIdToRecoTSId_Map[hitId].end())
            recoFraction = found_reco->second;
          numerator_simToReco[i][j] += std::min(simFractionSquared, (simFraction - recoFraction)*(simFraction - recoFraction))*hitEnergySquared;
  ...
  for(unsigned int i = 0; i < nTracksters; ++i)
  {
    ...
    for(const auto& v: lcsInTrackster)
    {
      for(const auto& haf: layerClusters[v].hitsAndFractions())
      {
        ...
        auto found = std::find_if(detIdToRecoTSId_Map[hitId].begin(), detIdToRecoTSId_Map[hitId].end(), [=](const std::pair<int, float>& v) { return v.first == static_cast<int>(i); });
        ...
        for(size_t j = 0; j < nSimTracksters; ++j)
        {
          ...
          auto found_sim = std::find_if(detIdSimTSId_Map[hitId].begin(), detIdSimTSId_Map[hitId].end(), [=](const std::pair<int, float>& v) { return v.first == static_cast<int>(j); });
          if (found_sim != detIdSimTSId_Map[hitId].end())
            simFraction = found_sim->second;
          numerator_recoToSim[i][j] += std::min(recoFractionSquared, (simFraction - recoFraction)*(simFraction - recoFraction))*hitEnergySquared;
...
hgcal::RecoToSimCollectionSimTracksters TSToSimTSHitLCAssociatorByEnergyScoreImpl::associateRecoToSim(
  ...
  for (std::size_t tsId = 0; tsId < recoToSim_sharedEnergyAndScore.size(); ++tsId) {
    ...
    LogDebug("TSToSimTSHitLCAssociatorByEnergyScoreImpl") << " Trackster Id:\t" << tsId << "\tSimTrackster id:\t"
                                                       << recoToSim_sharedEnergyAndScore[tsId][simTsId].first << "\tscore:\t" << recoToSim_sharedEnergyAndScore[tsId][simTsId].second << "\n";
      // Fill AssociationMap
...
hgcal::SimToRecoCollectionSimTracksters TSToSimTSHitLCAssociatorByEnergyScoreImpl::associateSimToReco(
  ...
  for (std::size_t simTsId = 0; simTsId < simToReco_sharedEnergyAndScore.size(); ++simTsId) {
    ...
    {
      LogDebug("TSToSimTSHitLCAssociatorByEnergyScoreImpl") << "Trackster Id:\t" << tsId << "\tSimTrackster id:\t" << simTsId << " Shared energy "
                                                       << simToReco_sharedEnergyAndScore[simTsId][tsId].first << "\tscore:\t" << simToReco_sharedEnergyAndScore[simTsId][tsId].second << "\n";
      // Fill AssociationMap
