...
ticl::LayerClustersAndAssociations HGCalCLUEAlgoT<T, STRATEGY>::getClusters(bool) {
  ...
  for (unsigned int layerId = 0; layerId < 2 * maxlayer_ + 2; ++layerId) {
    ...
    std::cout << __LINE__ << std::endl;
    auto const detidsize = cells_[layerId].detid.size(); 
    std::cout << __LINE__ << std::endl;
    for(unsigned int idx = 0u; idx < detidsize; idx++){
      std::cout << __LINE__ << std::endl;
      assert(idx < detidsize);
      std::cout << __LINE__ << std::endl;
      auto const detid = cells_[layerId].detid[idx];
      std::cout << static_cast<uint32_t>(detid) << std::endl; 
      std::cout << __LINE__ << std::endl;
    }
